...
export const ViewZones: React.FC = () => {
    const [searchQuery, setSearchQuery] = useState('')
    const [debouncedSearch, setDebouncedSearch] = useState('')
    const [currentPage, setCurrentPage] = useState(1)
    const [editingId, setEditingId] = useState<number | null>(null)
    const [editValue, setEditValue] = useState('')
    const [zones, setZones] = useState(mockZonesData)
    ...
    const filteredData = zones.filter(zone =>
        zone.zoneName.toLowerCase().includes(debouncedSearch.toLowerCase())
    )
    ...
    const handleEdit = (zone: typeof mockZonesData[0]) => {
        setEditingId(zone.id)
        setEditValue(zone.zoneName)
    }

    const handleSave = (id: number) => {
        setZones(zones.map(zone =>
            zone.id === id ? { ...zone, zoneName: editValue } : zone
        ))
        setEditingId(null)
        setEditValue('')
    }
    ...
    const handleAddNew = () => {
        ...
        const newZone = { id: newId, zoneName: 'New Zone' }
        setZones([...zones, newZone])
        setEditingId(newId)
        setEditValue('New Zone')
    }

    const handleExport = () => {
        const csv = [
            ['S.No.', 'Zone Name'],
            ...filteredData.map((zone, index) => [index + 1, zone.zoneName])
                                    ...
                                    >
                                        <div className="flex flex-col items-center gap-2">
                                            <p className="text-sm font-medium text-[#757575]">
                                                {debouncedSearch ? 'No zones found matching your search' : 'No zones available'}
                                            </p>
                                            {debouncedSearch && (
                                                <button
